...
public class OpenControllerTest {
    ...
    @DisplayName("회원가입 없이 /api/open 호출 후 200 응답 확인")
    @Test
    public void testOpenAccess() throws Exception {
        mockMvc.perform(get("/api/open"))
                .andDo(print()) // Log the response
                .andExpect(status().isOk());
    }

    @DisplayName("ROLE_ADMIN, ROLE_MANAGER, ROLE_USER 3명 회원 가입 후 /api/open/required/1 호출 후 각각 레벨에 맞게 200이나 403 상태코드 응답 확인")
    @Test
    public void testRoleLevel1Access() throws Exception {
        // Call the /api/open/required/1 API with admin token
        mockMvc.perform(get("/api/open/required/1")
                        .header("Authorization", "Bearer " + adminToken))
                .andDo(print()) // Log the response
                .andExpect(status().isOk());

        // Call the /api/open/required/1 API with manager token
        mockMvc.perform(get("/api/open/required/1")
                        .header("Authorization", "Bearer " + managerToken))
                .andDo(print()) // Log the response
                .andExpect(status().isForbidden());

        // Call the /api/open/required/1 API with user token
        mockMvc.perform(get("/api/open/required/1")
    ...
    @DisplayName("ROLE_ADMIN, ROLE_MANAGER, ROLE_USER 3명 회원 가입 후 /api/open/required/2 호출 후 각각 레벨에 맞게 200이나 403 상태코드 응답 확인")
    @Test
    public void testRoleLevel2Access() throws Exception {
        // Call the /api/open/required/2 API with admin token
        mockMvc.perform(get("/api/open/required/2")
                        .header("Authorization", "Bearer " + adminToken))
                .andDo(print()) // Log the response
                .andExpect(status().isOk());

        // Call the /api/open/required/2 API with manager token
        mockMvc.perform(get("/api/open/required/2")
                        .header("Authorization", "Bearer " + managerToken))
                .andDo(print()) // Log the response
                .andExpect(status().isOk());

        // Call the /api/open/required/2 API with user token
        mockMvc.perform(get("/api/open/required/2")
                        .header("Authorization", "Bearer " + userToken))
                .andDo(print()) // Log the response
                .andExpect(status().isForbidden());
    ...
    @DisplayName("ROLE_ADMIN, ROLE_MANAGER, ROLE_USER 3명 회원 가입 후 /api/open/required/3 호출 후 각각 레벨에 맞게 200이나 403 상태코드 응답 확인")
    @Test
    public void testRoleLevel3Access() throws Exception {
        // Call the /api/open/required/3 API with admin token
        mockMvc.perform(get("/api/open/required/3")
                        .header("Authorization", "Bearer " + adminToken))
                .andDo(print()) // Log the response
                .andExpect(status().isOk());

        // Call the /api/open/required/3 API with manager token
        mockMvc.perform(get("/api/open/required/3")
                        .header("Authorization", "Bearer " + managerToken))
                .andDo(print()) // Log the response
                .andExpect(status().isOk());

        // Call the /api/open/required/3 API with user token
        mockMvc.perform(get("/api/open/required/3")
                        .header("Authorization", "Bearer " + userToken))
                .andDo(print()) // Log the response
                .andExpect(status().isOk());
